...
"""
# Gemma with CoT suppression
python LLMS/eval_ollama_baseline.py \
  --models gemma3:1b \
  --intervention suppress_cot \
  --temperature 0

# Qwen with counterfactual intervention
python LLMS/eval_ollama_baseline.py \
  --models qwen3:8b \
  --intervention counterfactual_check \
  --temperature 0

# Compare both models under same intervention
python LLMS/eval_ollama_baseline.py \
  --models gemma3:12b qwen3:8b \
  --intervention label_then_reason \
  --temperature 0

# Add your own custom intervention text
python LLMS/eval_ollama_baseline.py \
  --models qwen3:8b \
  --intervention induce_cot \
  --intervention_text "Prioritize lexical triggers and causal structure over topic keywords." \
  --temperature 0
...
def parse_args() -> argparse.Namespace:
    # example usage:
    # python LLMS/eval_ollama_baseline.py --models qwen3:4b         
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "suppress_cot", "induce_cot", "counterfactual_check", "label_then_reason"],
        help=(
            "Thought-intervention mode applied in system/user prompts. "
            "Useful for reasoning-heavy models like Gemma/Qwen."
        ),
    )
    parser.add_argument(
        "--intervention_text",
        type=str,
        default="",
        help="Optional custom intervention text appended to the system instruction.",
    )
...
def intervention_instructions(mode: str) -> str:
    if mode == "none":
        return ""
    ...
    if mode == "suppress_cot":
        return (
            ...
            "Return only the final JSON object with a short reason."
        )

    if mode == "induce_cot":
        return (
            "Think step-by-step internally before deciding, but do not expose hidden reasoning. "
            "Return only final JSON."
        )

    if mode == "counterfactual_check":
        return (
            "Before finalizing, internally test at least one plausible alternative label and reject it if weaker. "
            "Return only final JSON."
        )

    if mode == "label_then_reason":
        return (
            "First decide a label internally, then produce a one-sentence reason grounded in the text. "
            "Return only final JSON."
        )
...
def build_system_prompt(intervention_mode: str, intervention_text: str) -> str:
    parts = [
        "You are a precise classifier.",
        "Follow instructions exactly and return only JSON.",
    ]

    mode_text = intervention_instructions(intervention_mode)
    if mode_text:
        parts.append(mode_text)

    if intervention_text.strip():
        parts.append(intervention_text.strip())
...
def extract_json_obj(text: str) -> dict[str, Any] | None:
    ...
    # Some reasoning-tuned models emit hidden/thinking blocks around answers.
    ...
    for model in args.models:
        ...
        for split_name, split_df in (("dev", dev_df), ("test", test_df)):
            metrics, pred_df, _ = run_model_on_split(
                ...
                intervention_mode=args.intervention,
