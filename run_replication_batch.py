...
"""
python run_replication_batch.py --num_runs 5 --max_attempts 10 --cuda_visible_devices 0
"""
...
def _find_prediction_files(run_dir: str) -> list[str]:
    return sorted(glob.glob(os.path.join(run_dir, "**", "predictions_*.csv"), recursive=True))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run paper_replication.py multiple times and aggregate metrics across runs."
    )
    parser.add_argument(
        "--num_runs",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            "Any arguments after '--' are passed through to paper_replication.py. "
            "Example: -- --training_mode few-shot --few_shot_mode train"
        ),
    )
